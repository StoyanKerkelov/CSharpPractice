...
class MortgageAccount : BankAccount
{
    ...
    public override decimal CalculateInterest(int months)
    {
        if (this.CustomerType == Customer.Company)
        {
            if (months <= 12)
            {
                return this.AccountBalance * (this.InterestRate / 2) * months; // Calculates half the rate for 1st 12 months
            }
            else
            {
                return (this.AccountBalance * (this.InterestRate / 2) * months) +  //Calculates half rate for first year plus normal interest rate 
