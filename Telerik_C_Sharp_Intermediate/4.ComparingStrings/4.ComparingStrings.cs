...
namespace _4.ComparingStrings
{           //Task: Implement lexicographic comparison without using the built-in .NET functionality. 
            //Provide feature to ignore string cases.
    class Startup
    {
        ...
        static string[] OrderLexicographically(string left, string right, bool ignoreCase = true)
        {
            ...
            {
                // to ignore case, it would be best to ingore the case here, not to create two new strings in the beggining
                if (left[i] < right[i])
                {
                    result[0] = left;
                    result[1] = right;
